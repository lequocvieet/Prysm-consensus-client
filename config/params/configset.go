...
func (r *configset) add(c *BeaconChainConfig) error {
	...
	//log.Infof("Fork version schedule: %+v", c.ForkVersionSchedule)
	for _, value := range r.versionToName {
		log.Infof("value of version to name: %+v", value)
	}
